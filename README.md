...
| `autoInstallSkill` | `true` | 插件加载时把随件的「围棋详细讲解」技能装进 `<DSH_HOME>/skills`（备份后覆盖、逐件复验）。关掉＝不碰技能目录，并且不会要求模型去加载那个技能。见 [内置讲棋技能](#内置讲棋技能随件发行) |
...
**从 v0.2.11 起，插件加载时会自动把它装进技能根**，并在人设里要求模型在讲棋前先加载它——所以这份判据不是"随包躺着"，而是会被真正用上。下面两节分别说清「怎么装」与「模型怎么被要求用它」。
...
| 盘面锚 | **89 条可回放案例**（v0.2.12 起）：每条挂着一份现成 SGF，把「书上的这个形」钉在具体坐标上（见下节） |
| 边界 | 技能**不出图、不自算数值、不判死活精确结论**——这三类一律让位给上面的 `go_*` 工具（见技能内 `BOUNDARIES.md` §2） |

### 盘面锚：把判据钉在坐标上（v0.2.12 起）

判据都是人话（「白 1 顶是俗手，正着在 A 位单粘」），可 **A 位是哪儿？** 原始判据里只有文字，讲棋时指不出来。所以这一版给技能加了一层 **盘面锚**：从三套**自带讲解的 SGF**（《欺招的攻与防》58 局 ／《围棋妙手百例》101 局 ／《吴清源鬼手 30 题》30 局，共 189 个文件、8056 着、2772 条讲解）里抽出 **89 条讲解 ↔ 具体坐标**能机械对齐的案例，每条只在需要时按需加载：

| | 内容 |
|---|---|
| 命名空间 | `SG-001`…`SG-089`（独立于判据池的 `Vx-nnn`，互不干扰） |
| 两条索引 | `ANCHORS.md`（人读：坐标 + 引文 + 讲解 + 出处）；`ANCHORS.jsonl`（机读：每行带一份**现成 SGF**） |
| 回放方式 | 技能正文新增 `## A1b 盘面锚（可回放案例库）`——讲到某条时把该锚的 SGF 交给 `go_draw_diagram` 出图，**图上的数字与书上一致** |
| 抽取口径 | 手数引用（「黑 7」「白 9」）**884 处机械验证 783 处（88.6%）**，坐标引用 **36/36**；未通过机械验证的一律不入库，存疑的 101 条写进 `anchor-mismatch.json` 留档 |

判定标准是**不用人肉读图**：同一节点上「文字坐标 ↔ SGF 标记（TR/SQ/CR/LB）↔ 棋子颜色 ↔ AB/AW 摆子」四者一致才算锚。样例：欺招 01 图 2 原文「黑 Q10 一子」，该节点带 `TR[pj]`（＝Q10）且题面 `AB` 里 pj 就是黑子。

> 为什么不是「让模型读棋盘图」：源书 PDF 是纯扫描件（无文本层），当年 OCR 只救回了文字、棋盘图全丢了。这一版的思路是**换一条源料**——用本来就有棋谱的 SGF 把坐标找回来，而不是去猜图。

### 怎么装（v0.2.11 起：装插件就自动装，不用手动）

技能的运行时位置是 **DSH 技能根**（`<DSH_HOME>/skills`），而随件在**插件包里**——两者不是同一个地方。从 v0.2.11 起这一步由插件自己做：

**插件每次加载时**（即每次启动 `dsh web`）会检查技能根，缺了就装、内容旧了就**备份后覆盖**（旧文件留成 `.foo.md.bak-<时间戳>`），然后逐件比 SHA256 复验。所以正常安装流程下你**什么都不用做**：

```
装插件 → 启动 dsh web → 技能就位 → 下一个新会话里模型就能加载它
```

> 注意时序：技能是在**会话启动时**被引擎读进目录的，所以插件装好的技能要**新开一个会话**才可见（当前开着的会话看不到）。这不是 bug，是"技能目录在会话开始时快照"的必然结果。

配置项（默认开）：

```yaml
autoInstallSkill: true    # false = 不碰技能目录，同时也不会要求模型去加载那个技能
```

**为什么不是 npm 的 postinstall**：DSH 的插件常用 `link:` 装（本地开发/自改源码时就是），pnpm 对 `link:` 依赖**不执行被链接包的 install 脚本**，postinstall 根本不会跑。加载期是唯一可靠的时机。

### 模型怎么被要求用它

技能**可用**时（技能根里确实有 `SKILL.md`），插件会在「围棋老师」人设里多注入一条：

> **凡是要讲解一手棋、复盘、或回答"这手为什么不好／该怎么下／这个形好不好"之前，先调用技能工具加载 `go-detailed-explanation`**，再按它的判据与固定五段结构组织讲解；判据与原文出处以该技能为准，不要凭印象讲。只查谱、只要数值、只要配图时**不需要**加载。

三条边界写清楚：① 只在**技能确实可用**时注入——不然就是在要求模型加载一个不存在的东西；② 触发面只盖**讲棋/复盘**类请求，纯查谱/数值/出图不要求（不白花 token）；③ `autoInstallSkill: false` 时既不装、也不注入。

### 手动通道（预览与核对）

自动安装发生在加载期，想**现在就**看装了什么、或者装到别的技能根，仍可以用这个命令：
...
node scripts\install-skill.mjs --force             # 目标已存在时覆盖（默认拒绝，退出码 3）
```

脚本与自动安装**共用同一份实现**（`src/skill-install.js`）——所以"命令行装好了、插件那条路径没装"这种分叉不会发生。它会打印源、目标与每个文件的 SHA256 并逐件比对，**不一致即视为未装成**；退出码 `0` 装成／`1` 出错／`2` 用法错误／`3` 目标已存在且未加 `--force`（本次未安装）。

> 若技能根里已有你自己改过的同类技能：自动安装会**备份后覆盖**（技能随插件升级）；手动命令默认**不覆盖**，要看差异自己 diff，确认要覆盖加 `--force`。
