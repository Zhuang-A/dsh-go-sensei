...
- **跟随讲解**（默认开）：Sensei 在对话里读到哪一盘、讲到第几手，棋盘就自动切过去——它调用 `go_position_context` 看第 42 手，面板的棋盘就停在第 42 手的局面。只在面板与棋盘都展开、且开关打开时才每 3 秒问一次宿主（纯内存查询，不读盘）。
...
- **「跟随讲解」是进程级的一个指针**：最近一次 `go_*` 调用（读谱 / 找问题手 / 取局面 / 写回注释）决定它指向哪一盘、第几手。多个会话同时复盘时，指针会被最后一次调用覆盖；关掉面板上的「跟随讲解」即可纯手动控制。
- **改了 `client.js` 要重启 `dsh web`**：浏览器 half 的产物在插件激活时被读取一次（宿主按激活时刻快照），刷新页面不够。
