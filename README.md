...
没有分析数据的棋谱要现场补算（本机实测 96 手约 **29 秒**）。补算完成后，插件会把**逐手胜率与目差写回棋谱**的 KataGo 属性：
...
于是同一份棋谱**再打开就是秒开**（面板、整页棋盘、右侧栏、`go_review_moves` 都不再触发补算），别的打谱软件打开也能直接看到胜率数据。写回只发生在**明确的分析动作**里（`go_review_moves` 的自动补算、或显式调用 `go_engine_analyze`），只读的面板路由从不改文件；同一进程内还留了一份内存缓存，模型刚算过的棋谱面板直接复用。
