...
- **Web 面板里带一块可收起的棋盘**：面板列出问题手，点一行→棋盘跳到那一手（问题手彩点、AI 首选青圈、变化图半透明棋子带序号、讲解小方点；**AI 首选与变化图对每一手都在，讲解点也有**）；盘下带**图例与开关**（三类标注各自可关），Sensei 在对话里讲到哪一手，棋盘可以自动跟过去；棋盘还能开在**原生右侧栏**（对话左、棋盘右）或**左侧栏整页版**（[见下文](#web-页面上的复盘面板)）。
