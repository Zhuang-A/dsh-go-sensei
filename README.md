...
| **小黑块 / 小白块**（空点上） | **领地显示**（简易形势判断）：只挨黑子、只挨白子的空点各画一小块，黑白都挨的单官不画；盘下同时给出一行点目（见 [形势判断与领地显示](#形势判断与领地显示)） |
...
**图例就在棋盘下面**，每项＝色样 + 名称的小胶囊，**点一下即可开/关**那一类标注（关掉的项变淡划掉），四项分别是「问题手」「有讲解」「AI 首选 / 变化图」「领地 / 形势判断」。开关是三个视图共享的：在下方面板关掉，右侧栏与整页棋盘同时生效。只想安静看棋形时，把四类全关掉即可。
...
照 [Lizzieyzy](https://github.com/yzyray/lizzieyzy) 的「形势判断 + 领地显示」做的：**盘上把地画出来、盘下把目数报出来**，滑块拖到哪一手就按那一手的局面算。默认开启，在棋盘图例里点「**领地 / 形势判断**」可以整层关掉（盘上色块与盘下那行数字一起消失）。

怎么算的（只做一件事，规则很短）：

1. 把空点按四连通切成若干「空块」；
2. 只看每块空点**紧邻的棋子颜色**——只挨黑子＝黑地、只挨白子＝白地、黑白都挨或谁都不挨＝**单官**（不画也不计）；
3. 按**数子法**合计：黑 ＝ 黑子 ＋ 黑地，白 ＝ 白子 ＋ 白地 ＋ 贴目，正数表示黑领先。

盘下那行形如：

> 简易形势判断：黑 52 目 · 白 54 目（含贴目 7.5）· 白领先 9.5 目

**这是启发式估算，不是引擎的结论**：不提死子、不判双活、不认眼形，中盘大片未定型的地方会算成单官（因此中盘数字主要由盘上的子构成，地盘要封闭了才看得出来）。真要看精确的目差与胜率，用旁边那条**目差曲线**（引擎的 `DM`，黑方视角）——两者口径不同，讲解时也不该混着说，人设里已经写明了这一条。

配图也能用：`go_draw_diagram` 加 `territory: true`，生成的 SVG 上会叠同样的领地色块，图下附一行简易点目，工具返回值里也带着那行数字（模型应直接引用它，不要自己另算）。适合讲「这块地是谁的」「现在谁领先」。
...
- 想讲地盘归属或形势优劣时加 `territory: true`：盘上叠出**领地显示**（只挨黑子/只挨白子的空点画成黑/白小方块），图下附一行简易点目；
...
| `go_draw_diagram` | 画讲解配图：变化图（1-9 / A-Z 编号）+ 三角形等重点棋子标注；`territory: true` 可叠加领地显示与简易形势判断，返回可直接嵌入回答的 Markdown 图片行 | 需要 Web 面板（`dsh web`） |
...
- **领地显示／形势判断是启发式估算，不是引擎结论**：只按「空块紧邻的棋子颜色」判归属（只挨单色才算地、单官中立），**不提死子、不判双活、不认眼形**——中盘大片未定型处一律算单官，所以那些手数上的数字主要由盘上的子构成，地盘封闭了才看得出地。精确目差与胜率请看引擎的目差曲线（`DM[]`）。这两套数字口径不同，人设里已明确不许混着讲。领地层与那行点目可以随时在图例里关掉。
