...
![右侧栏 .sgf 文档预览：棋盘、控件、两条曲线，最底下是图例与讲解](docs/images/doc.png)

- 右侧栏是一列：棋盘在上，往下是控件条、两条曲线，最底下是**图例 + 「N 手 · M 个问题手」+ 讲解框** —— 讲解单独占一块（翻到哪手读哪手；这一手还没写回讲解时给一行提示），问题手列表就在这一列里继续往下滚。同样带 `⏮ ◀ ▶ ⏭` + `◀恶点/恶点▶` + 滑块；点某一行会跳到那一手并把追问语**复制到剪贴板**（右侧栏没有输入框，只能复制）。
- 侧栏窄、又要在同一列里塞下曲线，所以棋盘的宽度跟着窗口高度走：**窗口矮的时候棋盘自己收一点，讲解始终留在屏内**。
