...
- **Web 面板里带一块可收起的棋盘**：面板列出问题手，点一行→棋盘跳到那一手（问题手彩点、AI 首选青圈、变化图蓝点带序号、讲解小方点）；盘下带**图例与开关**（三类标注各自可关），Sensei 在对话里讲到哪一手，棋盘可以自动跟过去；棋盘还能开在**原生右侧栏**（对话左、棋盘右）或**左侧栏整页版**（[见下文](#web-页面上的复盘面板)）。
...
**图例就在棋盘下面**，每项＝色样 + 名称的小胶囊，**点一下即可开/关**那一类标注（关掉的项变淡划掉），三项分别是「问题手」「有讲解」「AI 首选 / 变化图」。开关是三个视图共享的：在下方面板关掉，右侧栏与整页棋盘同时生效。只想安静看棋形时，把三类全关掉即可。
...
没有分析数据的棋谱要现场补算（本机实测 96 手约 **29 秒**）。补算完成后，插件会把**逐手胜率、目差，以及 AI 首选与变化图**一起写回棋谱：
...
于是同一份棋谱**再打开就是秒开**（面板、整页棋盘、右侧栏、`go_review_moves` 都不再触发补算），别的打谱软件打开也能直接看到胜率数据与首选/变化图。写回只发生在**明确的分析动作**里（`go_review_moves` 的自动补算、或显式调用 `go_engine_analyze`），只读的面板路由从不改文件；同一进程内还留了一份内存缓存，模型刚算过的棋谱面板直接复用。

**早先复盘过的棋谱**（那时只写了 `WV[]`/`DM[]`）想补上首选与变化图，对它显式跑一次 `go_engine_analyze` 即可：`go_review_moves` 不会自作主张重算（它认为"已有分析"），显式补算才会把 `LZ[]` 一并写上。

写回与讲解回写共用同一套"重新序列化"逻辑，手数与旁支都不丢（幂等：重复写回不会堆积属性）。想保留原样就先备份——`backup/` 目录就是干这个的。

面板与对话走同一条管线（含自动补算），所以结果一致；**面板本身只读文件**（补算结果的写回只发生在工具里，见上一节），路径被限制在已知工作区目录之内。棋盘数据由 `/go-sensei/review` 与问题手一并返回（含 `AB`/`AW` 摆子与提子后的真实盘面），不必为画盘再读一次棋谱。

### 棋盘开在右边（原生右侧栏）

插件的第二个落点是**原生右侧栏**：为 `.sgf` 注册了一个文档预览实现（客户端 `ctx.documentPreviews`），所以在对话里点文件的「**打开**」、或从文件列表点开 `.sgf`，棋盘就出现在右侧栏 —— **对话留在左边，两边互不遮挡**。

- 右侧栏那块纵向排布：棋盘在上、问题手列表在下，同样带 `⏮ ◀ ▶ ⏭` + `◀恶点/恶点▶` + 滑块；点某一行会跳到那一手并把追问语**复制到剪贴板**（右侧栏没有输入框，不能像下方面板那样直接插入）。
...
输入框下方那块地方有限，想看大棋盘就点**左侧栏的「Sensei 棋盘」**：整页版把棋盘放大到左半边，右边是问题手的**详细说明**（胜率差 · 目差 · AI 首选及其胜率 · 变化图序列）。
...
- 整页版拿不到"插入输入框"的能力（那是输入框插槽独有的），所以那边**点一行＝把追问语复制到剪贴板**，粘到下面输入框回车即可；要一键插入就用输入框下方那块面板。
